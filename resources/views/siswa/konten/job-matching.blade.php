...
<main class="p-6">
    <div class="bg-white p-6 rounded shadow">
        <h2 class="text-lg font-semibold mb-4">Daftar Lowongan Job Matching</h2>

        @if ($sertifikatLulus < 2)
            <p class="text-center text-gray-500">Job Matching akan tersedia setelah kamu mendapatkan minimal 2 sertifikat lulus.</p>
        @else
            @if ($jobMatchings->isEmpty())
                <p class="text-center text-gray-500">Tidak ada lowongan tersedia saat ini.</p>
            @else
                                ...
                                <td class="py-2 px-4">
                                    @if (!isset($jobApplications[$job->id]))
                                        <form action="{{ route('siswa.job-matching.apply', $job->id) }}" method="POST">
                                            @csrf
                                            <button class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">Lamar</button>
                                        </form>
                                    @else
                                        <button disabled class="bg-gray-300 text-gray-600 px-3 py-1 rounded">Sudah Melamar</button>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
